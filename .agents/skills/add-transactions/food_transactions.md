...
- When a receipt prints a separate voucher, e-coupon, or cash-card discount line, record it as a distinct negative expense posting with a `food_or_drink:` comment and keep the payment posting separate. Preserve the exact receipt label for the discount when present (for example, `[電子印花卡憧]赩 $30早餐電子現金券`). This preserves the receipt structure and makes the discount auditable.
