...
- I/O: Accept `os.PathLike` for public API types; use `anyio.Path` or coerce `PathLike` to `anyio.Path` for internal async file operations (e.g., when using async `.open()` and other async path methods). When converting `PathLike` objects to `str` (for example, when passing paths to subprocesses or APIs that require plain strings), **always** use `os.fspath(path_like)` (or `from os import fspath`) instead of `str(path_like)` so the filesystem path protocol is correctly honored.
