...
- Keep `lessons.md` short and recent. Run `bun run format` and `bun run test` after changes.

## 2026-02-08

- When adding a payee, remember to place it in strict lexicographic order in `preludes/self.journal` and validate with `bun run format` and `bun run check`.
